...
TEMPLATE_TEST_CASE("PartitionedAffine", "", float, double) {
    ...
    SECTION("operator-=") {
        ...
        SECTION("Independent") {
            pinterval_t value(one, two);
            pinterval_t value_temp(three, four);
            value -= value_temp;
    ...
    SECTION("operator*=") {
        ...
        SECTION("Independent") {
            ...
            // Sharp answer is [3, 8]
            test_interval(value.range(), 2.972500, 8.022500);

            // pinterval_t value2(three, value_t(8.0));
            // value2 *= pinterval_t(interval_t(-three, -two));
            // std::cout << value2.print_interval_form() << std::endl;
            // // Sharp answer is [-24, -6]
            // test_interval(value2.range(), -23.752500, -2.085000);

            // value *= pinterval_t(interval_t(-two, two));
            // std::cout << value.print_interval_form() << std::endl;
            // Sharp answer is [-48, -12]
            // test_interval(value.range(), value_t(-48.0), value_t(-12.0));
        }
        SECTION("Dependent") {
            pinterval_t value(-one, two);
            value *= value;
    ...
    SECTION("operator/=") {
        ...
        //     std::cout << value.print_interval_form() << std::endl;
        //     std::cout << value.range().print_interval_form() << std::endl;
        ...
        //     test_interval(value.traditional_interval(), 0.909008, 1.100100);
        // }
    }
}
